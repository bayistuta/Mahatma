angular.module('mahatma.controllers', [])
  ...
  .controller('AccountCtrl', function ($scope, dataStore, Utils, Constants, $ionicLoading) {
    var vm = this;
    vm.nickName = '';
    vm.address = '';
    vm.userType = '';
  ...
  .controller('AccountChangePasswordCtrl', function ($scope, dataStore, Utils, Constants, $ionicLoading, ionicToast) {
    var vm = this;
    vm.oldPassword = '';
    vm.newPassword = '';
    vm.confirmPassword = '';
    ...
    vm.isValid = false;
    vm.isConfirmPasswordPassed = true;

    function request() {
      $ionicLoading.show({
        template: '数据发送中...'
      });
      dataStore.changePassword(vm.oldPassword, vm.newPassword).then(
        function (data) {
          $ionicLoading.hide();
          if (data.ErrorItemList.length > 0) {
            var message = '';
            ...
            ionicToast.show('密码修改成功', 'top', false, 2500);
          }
        },
        function (data) {
          $ionicLoading.hide();
          ionicToast.show('密码修改失败，请确认你的旧密码', 'top', false, 2500);
          rest();
        }
      );
    }
    ...
    function validate() {
      vm.isConfirmPasswordPassed = true;
      if (vm.oldPassword.length > 0 && vm.newPassword.length > 0 && vm.confirmPassword.length > 0) {
        vm.isValid = true;
        if (vm.newPassword != vm.confirmPassword) {
          vm.isValid = false;
          vm.isConfirmPasswordPassed = false;
        }
      }
    }
  });
